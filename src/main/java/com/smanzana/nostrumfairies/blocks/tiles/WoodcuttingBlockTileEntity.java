...
public class WoodcuttingBlockTileEntity extends LogisticsTileEntity implements ITickable,  ILogisticsTaskListener, IFeySign {
		...
		private void makeChopTreeTask(BlockPos base) {
			...
			if (!taskMap.containsKey(base) && network.taskDataAdd(WoodcuttingBlock.WOODCUTTING_POSITION, base)) {
		...
		private void removeTask(BlockPos base) {
			...
			network.taskDataRemove(WoodcuttingBlock.WOODCUTTING_POSITION, base);
		}
		...
		public void onTaskComplete(ILogisticsTask task, IFeyWorker worker) {
			...
			if (task instanceof LogisticsTaskChopTree) {
				...
				if (taskMap.remove(pos) != null) {
					...
					LogisticsNetwork network = this.getNetwork();
					if (network != null) {
						network.taskDataRemove(WoodcuttingBlock.WOODCUTTING_POSITION, pos);
					}
				}
